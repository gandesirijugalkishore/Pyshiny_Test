...
from mesop import RadioChangeEvent
import mesop as me
...
def respond_to_chat(input, history):
    # Implement the logic for the Mesop Bot's response
    return ["Mesop Bot's response to: " + input]

_CHAT_MAX_WIDTH = "800px"
_MOBILE_BREAKPOINT = 640

# Agent definitions (replace with your actual agents)
_AGENTS = {
    "Mesop Bot": respond_to_chat,  # Default Mesop bot
    "Agent 2": lambda input, history: ["Agent 2's response to: " + input],  # Example agent
    "Agent 3": lambda input, history: ["Agent 3 says: " + input],  # Example agent
}
...
class State:
  ...
  selected_agent: str = "Mesop Bot"  # Add selected_agent to the state
...
def agent_dropdown():
    ...
    me.select(
        label="Select Agent",
        options=[
            me.SelectOption(label=agent, value=agent) for agent in _AGENTS.keys()
        ],
        on_selection_change=on_agent_select,  # Call on_agent_select on change
        style=me.Style(width=100, margin=me.Margin(top=40)),  # Add some margin
        multiple=False,
        appearance="outline",
  ...
  with me.box(
    ...
    with me.box(style=me.Style(display="flex", gap=20)):
      ...
      if state.sidebar_expanded:
        agent_dropdown()  # Agent selection dropdown
